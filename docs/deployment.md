...
| `JOBTRACKER_RESUME_TEX` | For `tailor` | Your resume's LaTeX source. Personal data, like `answers.yaml`, so it is not in the image — mount it beside that. Absent → the `tailor` task reports itself unavailable and the rest of the queue runs |
| `JOBTRACKER_TAILORED` | No | Where assembled resumes are written. Defaults to `./data/tailored`, so a mounted `/data` covers it |
| `JOBTRACKER_SUBMISSIONS` | **In a container, yes** | Where the documents you actually submitted are archived. Defaults to `./data/submissions`. The one directory here nothing can rebuild — inside an image layer it is deleted by the next `docker pull`, taking the only copy of what went out under your name. `Dockerfile.serve` sets it |
| `JOBTRACKER_DOWNLOADS` | `./downloads.yaml` | Where each saved document goes (docs/downloads.md). Absent means all four fall back to the four variables below and then to `~/Downloads`. Personal config, gitignored; mount it beside `answers.yaml` |
| `JOBTRACKER_DOWNLOAD_RESUME`, `_LETTER`, `_RESUME_TEX`, `_LETTER_TEX` | No | The fallback destination for each kind, under anything `downloads.yaml` sets. Worth setting in a container, where `~/Downloads` is a directory in a layer nobody will look in |
