...
The long-form version of `CLAUDE.md` as it stood on 2026-09-10, before that file was cut
back to the rules alone. Nothing here is loaded into an agent's context by default.

Every *rule* in this file survives in `CLAUDE.md`. What was moved here is the evidence
behind them: the dates, the measurements, the failure narratives, and the per-incident
reasoning. Read it when you want to know *why* a rule exists, or before arguing that one
should be relaxed — most of them were written the day something broke silently.
...
`JOBTRACKER_PREFILL=1`. Applications are typed by hand. Everything in this section and
in "Growing the answer bank" and "The mirrored form" is **mothballed, not deleted**, and
the rules below still hold for the day it comes back. What the switch does, and why each
piece is where it is:

- **The hard guard is `browser.fill_application`,** not the UI. It is the one function
  that launches a browser at a third party's form, and a switch enforced only at the two
  callers is one the next caller walks past. `unavailable_reason()` reports the switch
  *before* the Playwright import, because "switched off" and "not installed" call for
  opposite responses and `playwright install` is the wrong repair for a decision.
- **`prepare` still rescores and still exits 0.** Rescoring is what stops `today`
  opening on yesterday's order, and it needs nothing from this half. Reporting NOT READY
  about three picks nobody will prefill would exit 2 nightly for a decision — the dbt
  Labs trap through a new door — so it prints the picks and stops. `prefill` and
  `apply-to` say so and exit 0, following `work --task <off>`: a feature you switched off
  is the system doing what you said, never an error.
- **The picks render no prefill line at all**, not "prefill: off" and not the "no prefill
  yet — run `jobtracker prefill`" absence. `build_dashboard` passes `plans=None` rather
  than querying, so both worlds look the same from below and the switch — read once for
  the page — is what tells them apart. An instruction you cannot follow, on three cards,
  every day, is the noise `_tailor_line` refuses to print about a feature nobody enabled.
- **Refusals name the switch first, before anything else that is missing.** `_api_apply_to`
  checks it before the posting, the answer bank and the browser; `prefill.unavailable_reason`
  before `answers.yaml`. That is the disabled-plugin rule: a reason printed beside a
  decision you typed reads as a fault, and points at the wrong thing to fix.
- **`_api_apply_to` is the only thing that starts a `live.Session`,** so every
  `/api/session/*` endpoint is inert by construction rather than by nine more guards.
  `/apply` says so itself, because "no window is open" is true and reads as a working
  feature nobody started.
- **Stored plans, gaps and learned question keys are kept.** Settings keeps the count in
  its heading and stops rendering the cards: answering a question for a fill that will not
  happen is the one thing that page should not ask. Nothing sweeps them — this is a
  decision that reverses.
- **The tests describe the on world; `tests/conftest.py` forces it.** An autouse fixture
  switches the feature on for the whole suite, the `sdk_installed` rule, so mothballed code
  keeps its coverage instead of rotting into something nobody can vouch for.
  `tests/test_prefill_switch.py` is the off world, including the default itself — which
  the suite could otherwise never see.
- **The serve image keeps Playwright and tectonic.** Turning a feature off is not a
  deployment change, and CI still asserts `browser.unavailable_reason() is None` — with
  the switch off that assertion would fail, so **CI sets `JOBTRACKER_PREFILL=1`**; a
  capability lost in an image must not look like a working deployment.
...
**Switched off with the rest of the prefill half (2026-09-10). The bank, its aliases and
every open gap are kept in place — see "Prefilled applications" for why nothing sweeps
them.**
...
**Switched off with the rest of the prefill half (2026-09-10) — see "Prefilled
applications". Mothballed, not deleted; every rule below still holds.**
