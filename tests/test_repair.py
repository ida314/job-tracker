...
def test_the_writer_changes_only_the_targeted_entry(companies_file):
    ...
    from jobtracker.curation import rewrite_companies as _rewrite_companies
    from jobtracker.migrate import _HEADER
...
def test_the_diff_comes_from_the_same_renderer_that_writes(companies_file):
    ...
    from jobtracker.curation import companies_diff as _companies_diff, rendered_companies as _rendered_companies
...
def test_inline_comments_block_a_write(companies_file):
    ...
    added tomorrow."""
    from jobtracker.curation import has_inline_comments as _has_inline_comments
...
# -- judge_board: one rule body, two entry points ------------------------------------
def test_a_typed_slug_is_judged_without_the_unchanged_shortcut():
    """`judge_candidate` opens with "is this the slug we already have?", which for a
    company built out of a form is always true — the candidate IS the entry. So the add
    form calls `judge_board`, and this pins that it actually applies the rule rather than
    short-circuiting."""
    result = FetchResult(
        company="Acme", ats="greenhouse", slug="acme", ok=True, status_code=200,
        observed_board_name="Someone Else",
        postings=[Posting("Acme", "1", "Mortgage Analyst", "u", "NY", None)],
    )
    v = repair.judge_board("Acme", "greenhouse", "acme", result, weak_evidence="reachable")
    assert v.accepted is False
    assert v.reason == "wrong_company"


def test_the_weak_label_is_the_callers_word_not_this_modules():
    """Only Greenhouse publishes a board name. For the other adapters the evidence is
    whatever the caller can honestly claim: `provenance` when a careers page served the
    link, `reachable` when somebody typed it. Hard-coding one would let a form-typed slug
    inherit a claim nobody made."""
    result = FetchResult(
        company="Acme", ats="ashby", slug="acme", ok=True, status_code=200,
        observed_board_name="acme",
        postings=[Posting("Acme", "1", "Backend Engineer", "u", "NY", None)],
    )
    assert repair.judge_board("Acme", "ashby", "acme", result,
                              weak_evidence="reachable").evidence_kind == "reachable"
    assert repair.judge_board("Acme", "ashby", "acme", result,
                              weak_evidence="provenance").evidence_kind == "provenance"


def test_no_identity_still_precedes_wrong_company_through_the_new_entry_point():
    """identity_matches returns True when either side is empty, so a blank board name
    must be caught before the comparison — otherwise "the identity endpoint 500'd" reads
    as "verified". The ordering has to hold for both callers, not just the old one."""
    result = FetchResult(
        company="Acme", ats="greenhouse", slug="acme", ok=True, status_code=200,
        observed_board_name="",
        postings=[Posting("Acme", "1", "Backend Engineer", "u", "NY", None)],
